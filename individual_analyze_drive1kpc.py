...
plt.plot(time,rhov3L,'k-.',label=r"$\rho v^{3} / 2L$")
plt.axhline(y=3e-5/cellVol1, color='r', linestyle='-',label="Driving dE/dt")
plt.plot(time,get_deriv(totE_codeunits,time,smooth=2000),linewidth=1,label="Total dE/dt")
plt.plot(time,get_deriv(np.array(ectot)/edenstocgs,time,smooth=2000),linewidth=1,label=r"dE$_{CR}$/dt")
#plt.semilogy(time,get_deriv(np.array(ketot)/edenstocgs,time,smooth=20000),linewidth=1,label=r"dE$_{k}$/dt")
plt.plot(time,get_deriv(np.array(metot)/edenstocgs,time,smooth=20000),linewidth=1,label=r"dE$_{B}$/dt")
plt.ylim(-5e-5,5e-5)
plt.xlabel(r't',fontsize=18)
plt.ylabel('dE/dt',fontsize=18)
plt.title(r"$P_{CR}/P_{g} \sim 1$",fontsize=22)
...
ecderiv = get_deriv(np.array(ectot)/edenstocgs,time,smooth=2000)
totederiv = get_deriv(np.array(totE_codeunits),time,smooth=200)
print("f_cr : ")
print(np.mean(ecderiv[int(len(ecderiv)/2):len(ecderiv)-1])/(3e-5/cellVol1))
print("f_E : ")
print(np.mean(rhov3L[int(len(ecderiv)/2):len(ecderiv)-1])/(3e-5/cellVol1))
print("f_tot (B + CR + KE) : ")
print(np.mean(totederiv[int(len(totederiv)/2):len(totederiv)-1])/(3e-5/cellVol1))
