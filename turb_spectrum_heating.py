...
def heating(field,data):
          ...
          sigma1 = (data['alfven_speed']*np.sqrt(1.e-7)*YTQuantity(1,"s/cm"))*(1./(1./data['Sigma_adv1'] + 1./data['Sigma_diff1']))
          s1 = data['Fc1'] - (4./(3.))*data['Ec']*vb/vmax
          return np.abs(sigma1*s1)*8.0/(3e-6)
         # return np.abs(data['Fc1']/data['Ec'])


yt.add_field(("gas","heating"), function=heating,sampling_type="local",display_name="Streaming Energy Loss Rate",units="")
...
def fft_comp(ds, iu, level, low, delta ):
   ...
   # ru = np.fft.fftn(rho**nindex_rho * u)[0:nx//2+1,0,0]
    ru = np.fft.fftn(u**0.5)[0:nx//2+1,0:ny//2+1,0:nz//2+1]
...
print("pcr/pg = 1, beta = 1, res = 512, lower Mach, streaming: ")
print(avg)
print(mina)
print(maxa)
print("k: ")
...
plt.legend(loc="lower right")
plt.tight_layout()
plt.savefig("heating_spectrum_streaming_lowerMach_res512_pcpg1_beta1.pdf")
plt.close()
